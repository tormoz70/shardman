...
- [ ] Cargo workspace + shardman-core: PeriodSpec, period_id, FSM standby/active/sealed
- [ ] Migrations + axum server: bootstrap, shards admin, resolve write/read, CLUSTER_KEY
- [ ] Stats ingest + seal supervisor with CAS rotate and auto-promote
- [ ] shardman-agent (pg size heartbeat) + shardman-cli
- [ ] Prometheus metrics, docker-compose + Grafana dashboard, integration tests
- [ ] README, sharding-model, architecture, .ai/master-spec.yaml
